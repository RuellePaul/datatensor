...
async def run_task(task):
    try:
        if task.type == 'generator':
            ...
            generator.main(task.user_id, task.id, properties=task.properties)
    except errors.APIError as error:
        update_task(task.id, status='failed', error=error.message, ended_at=datetime.now())
        ...
        update_task(task.id, status='failed', error=message, ended_at=datetime.now())
        notification = NotificationPostBody(type=NotificationType('TASK_FAILED'), description=message)
    else:
        update_task(task.id, status='success', ended_at=datetime.now())
