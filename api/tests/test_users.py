...
class TestUsers:
    ...
    def test_invalid_get_user(self):
        response = client.get(f'{PREFIX}/users/{Store.user["id"]}', headers={'Authorization': None})
        assert response.status_code == 422

        response = client.get(f'{PREFIX}/users/{str(uuid4())}', headers={'Authorization': Store.access_token})
        assert response.status_code == 404

    def test_valid_get_user(self):
        response = client.get(f'{PREFIX}/users/{Store.user["id"]}', headers={'Authorization': Store.access_token})
        assert response.status_code == 200
        assert UserResponse.validate(response.json())

        response = client.get(f'{PREFIX}/users/{Store.admin_user["id"]}', headers={'Authorization': Store.access_token})
        assert response.status_code == 200
        assert UserResponse.validate(response.json())

        response = client.get(f'{PREFIX}/users/{Store.user["id"]}', headers={'Authorization': Store.admin_access_token})
        assert response.status_code == 200
        assert UserResponse.validate(response.json())
